...
function View() {
   ...
   const deleteUser = async (id)=>{
      // const config = {
      //   headers: {
      //     authorization: `CodeLab Trainning ${currentUser?.data.token}`,
      //   },
      // };
    await axios.delete(`https://zzsolution.herokuapp.com/api/product/del/${id}`)
    const oppdata = [...dataFile]
    oppdata.filter(de => de.id !== id )
    setDatafile(oppdata)
     window.location.reload()
    console.log("use has been delete")
